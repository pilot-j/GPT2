...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.transformer = nn.ModuleDict(dict(
            ...
            h = nn.ModuleList([TF_Block(config) for _ in range(config.n_layer)]),
    ...
    def forward(self, idx, targets=None):
        ...
        pos = torch.arange(0, T, dtype=torch.long, device=idx.device) # you mark positions, shape (T)
        ...
        logits = self.lm_head(x) #for every seq we generate a vector that represents what word should come next (B, T, vocab_size)
        loss = None
    ...
    def from_pretrained(cls, model_type):
        ...
        # We will now try to copy param by param the weights as stored in the HF implementation
        sd = model.state_dict()
        sd_keys = sd.keys()
        sd_keys = [k for k in sd_keys if not k.endswith('.attn.causal_mask')] # discard this mask / buffer, not a param
